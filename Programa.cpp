...
//Evalua la funcion y su derivada en x(variable global)
...
//realiza la formula de Newton Raphson
void Newton_raphson(){
    Derivar_Polinomio();
    Evaluar_Funcion();
    if(dy == 0){
...
int main(int argc, char **argv){
    ...
    for (int i = 0; i < pol.length(); i++){
        if(i==0 && pol[i] == '+'){   //arreglar problema de inicio del polinomio con + o -
            temp = temp + pol[i];
        }
        else if(i==0 && pol[i] == '-'){
            temp = temp + pol[i];
